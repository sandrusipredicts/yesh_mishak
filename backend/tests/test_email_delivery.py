...
from app.services.email_delivery import EmailDeliveryError, ResendEmailDelivery, send_email
...
def configure_email_settings(monkeypatch):
    ...
    monkeypatch.setenv("PASSWORD_RESET_FROM_EMAIL", "reset@yesh-mishak.com")
...
def test_password_reset_adapter_uses_shared_resend_transport(monkeypatch) -> None:
    captured = {}

    def fake_post(url, **kwargs):
        captured.update(url=url, **kwargs)
        return FakeResponse(201, {"id": "reset-email-123"})

    monkeypatch.setattr("app.services.email_delivery.httpx.post", fake_post)
    result = ResendEmailDelivery().send_email(
    ...
    assert result.provider_message_id == "reset-email-123"
    assert captured["url"] == "https://api.resend.com/emails"
    assert captured["headers"]["Idempotency-Key"] == "reset-request-1"
    assert captured["json"]["from"] == "Yesh Mishak <reset@yesh-mishak.com>"


def test_password_reset_adapter_preserves_failure_result_contract(monkeypatch) -> None:
    monkeypatch.setattr(
        "app.services.email_delivery.httpx.post",
        lambda *_a, **_k: FakeResponse(503, {"message": "unavailable"}),
    )
    result = ResendEmailDelivery().send_email(
        to_email="user@example.com",
        subject="Reset password",
