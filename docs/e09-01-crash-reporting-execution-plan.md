...
| `@sentry/react` | `^10.60.0` | Frontend (peer of `@sentry/capacitor`) | Pinned to the exact version `@sentry/capacitor@4.2.0`'s `peerDependencies` declares (`10.60.0`), avoiding any peer-version ambiguity; compatible with the installed React `^19.2.6` (`@sentry/react`'s own peer range covers 16.14–19.x) |
...
- **Android**: `npx cap sync android` succeeded and regenerated the expected two files; `npm run build:android` (the exact step the CI workflow runs) succeeds. No physical/emulator APK install was performed in this environment (see §59.20).
- **YAML validation**: both modified GitHub Actions workflow files parse successfully as YAML (validated with `PyYAML`).
...
- Physical Android device crash/ANR verification (§49 of the plan) — no device available in this implementation environment; this is an explicit owner action, not skipped by oversight.
- All iOS verification (§47/§51 of the plan) — blocked on a macOS/Xcode toolchain and, beyond that, a physical iPhone; neither exists in this environment. Nothing iOS-related was claimed as verified anywhere in this report.
- Live Sentry dashboard verification (event actually appearing, correctly grouped, correct release/environment shown in the real Sentry UI) — requires the owner actions in §59.22 (a real Sentry org/project/DSN) to exist first; this implementation environment has no such external account.

### 59.22 Owner Steps Required

Full detail in [`docs/sentry-configuration-guide.md`](sentry-configuration-guide.md). Summary: create the Sentry organization and two projects (mobile, backend); obtain both DSNs and an auth token; add `VITE_SENTRY_DSN`/`SENTRY_ORG`/`SENTRY_MOBILE_PROJECT` (GitHub Actions repo variables) and `SENTRY_AUTH_TOKEN` (GitHub Actions secret); configure the equivalent values in Vercel (frontend) and Railway (backend) — neither is wired in-repo today since neither platform has any existing in-repo config to extend; configure the six alert rules in the Sentry dashboard; run the physical Android device verification pass; schedule the iOS active phase (macOS/Xcode access, physical iPhone).

### 59.23 External Sentry Configuration Still Required

No Sentry organization, project, DSN, or auth token exists yet for this project (none was created during this task, per the explicit prohibition). Every DSN/token reference in the code and CI workflows resolves to "absent," which is the designed safe-default state (monitoring disabled, everything else functions normally) — confirmed by the test suite (§59.19).
